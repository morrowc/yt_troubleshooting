...
def V4Mappings():
  ...
  m = MAPPING_RE.match(v4_mapping.decode())
  if m:
    map_addr = m.group(1)
  else:
...
def V6Mappings():
  ...
  m = MAPPING_RE.match(v6_mapping.decode())
  if m:
    map_addr = m.group(1)
  else:
...
def main():
  ...
  m = re.search(r'^Location:\s+http://(.+\.youtube\.com)/vid.*$', loc_v4.decode())
  if m:
    loc_v4 = m.group(1)
  ...
  if socket.has_ipv6:
    ...
    m = re.search(r'^Location:\s+http://(.+\.youtube\.com)/vid.*$', loc_v6.decode())
    if m:
      loc_v6 = m.group(1)
